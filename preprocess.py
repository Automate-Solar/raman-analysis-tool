"""Preprocessing pipeline using ramanspy for baseline correction, denoising, and normalization."""
...
def baseline_correct_als(
    ...
    """
    check_ramanspy()
    print("  Applying ALS baseline correction to the hypercube...")
...
def baseline_correct_ials(
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            if np.any(np.isfinite(spectrum.spectral_data)):
                try:
                    pipe = ramanspy.preprocessing.baseline.IALS(niter=niter)
                    corrected[i, j, :] = pipe.apply(spectrum).spectral_data
                except Exception as e:
                    print(f"  Warning: baseline correction failed at ({i}, {j}): {e}")
                    corrected[i, j, :] = spectrum.spectral_data
...
def baseline_correct_poly(intensity_cube: np.ndarray) -> np.ndarray:
    """Apply polynomial baseline correction using ramanspy.
    ...
    """
    check_ramanspy()
    print("  Applying polynomial baseline correction...")
    nx, ny, nw = intensity_cube.shape
    corrected = np.zeros_like(intensity_cube)
    ...
    for i in range(nx):
        for j in range(ny):
            ...
            if np.any(np.isfinite(spectrum.spectral_data)):
                try:
                    pipe = ramanspy.preprocessing.baseline.Poly()
                    corrected[i, j, :] = pipe.apply(spectrum).spectral_data
                except Exception as e:
                    print(f"  Warning: baseline correction failed at ({i}, {j}): {e}")
                    corrected[i, j, :] = spectrum.spectral_data
...
def main():
    ...
    if args.baseline != "none":
        ...
        elif args.baseline == "ials":
            intensity = baseline_correct_ials(intensity)
        print()
